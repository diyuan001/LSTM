...
def make_batch(train_path, word2number_dict, batch_size, n_step):
    ...
    for sen in text:
        ...
        if len(word) <= n_step:   #pad the sentence  希望输入长度为5 不足5，前面加空白pad（也可以加在后面
            word = ["<pad>"]*(n_step+1-len(word)) + word

        for word_index in range(len(word)-n_step):#n_step=5
...
def make_dict(train_path):
    ...
    number2word_dict[1] = "<unk_word>"#训练集中没有的单词
    word2number_dict["<sos>"] = 2
    number2word_dict[2] = "<sos>"#start
    word2number_dict["<eos>"] = 3
    number2word_dict[3] = "<eos>"#end
    ...
    def forward(self, X):
        ...
        for i in range(n_step):
            ...
            hidden_state = ot.mul(torch.tanh(
                cell_state))  # outputs : [n_step, batch_size, num_directions(=1) * n_hidden]

        # outputs, (_, _) = self.LSTM(X, (hidden_state, cell_state))
        #outputs, (_, _) = self.LSTM(X)
        # outputs : [n_step, batch_size, num_directions(=1) * n_hidden]
        # hidden : [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
        #outputs = self.hidden_state# [batch_size, num_directions(=1) * n_hidden]
        model = self.W(hidden_state) + self.b # model : [batch_size, n_class]
        #重置 h c
        # self.hidden_state.data = torch.zeros([batch_size, n_hidden]) # 记忆不随优化器迭代更新，在前馈中更新
        # self.cell_state.data = torch.zeros([batch_size, n_hidden]) # 记忆不随优化器迭代更新，在前馈中更新
        return model
