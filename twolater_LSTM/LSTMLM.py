...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        #输入门
        self.Wi = nn.Linear(n_hidden+emb_size, n_hidden, bias=True)  # 已加偏置
        self.Wc = nn.Linear(n_hidden+emb_size, n_hidden, bias=True)  # 已加偏置
        self.Wi2 = nn.Linear(2*n_hidden, n_hidden, bias=True)  # 已加偏置
        self.Wc2 = nn.Linear(2*n_hidden , n_hidden, bias=True)  # 已加偏置

        #遗忘门
        self.Wf = nn.Linear(n_hidden+emb_size, n_hidden, bias=True)  # 已加偏置
        self.Wf2 = nn.Linear(2*n_hidden , n_hidden, bias=True)  # 已加偏置

        #输出门
        self.Wo = nn.Linear(n_hidden+emb_size, n_hidden, bias=True)  # 已加偏置
        self.Wo2 = nn.Linear(2*n_hidden, n_hidden, bias=True)  # 已加偏置


    def forward(self, X):
        X = self.C(X)#嵌入

        hidden_state = torch.zeros( len(X), n_hidden)  # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
        cell_state = torch.zeros( len(X), n_hidden)     # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
        hidden_state2 = torch.zeros(len(X), n_hidden)  # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]
        cell_state2 = torch.zeros(len(X), n_hidden)  # [num_layers(=1) * num_directions(=1), batch_size, n_hidden]

        X = X.transpose(0, 1) # X : [n_step, batch_size, embeding size]
        #hx = torch.cat((self.hidden_state, X), 0)  # 拼接工作 [n_step+num_layers(=1) * num_directions(=1), batch_size, embeding size]
        for i in range(n_step):

            x1=X[i, :, :]
            hx = torch.cat((hidden_state, x1),1)  # 拼接工作 [batch_size, embeding size]
            # 输入门
            it = torch.sigmoid(self.Wi(hx))
            ct_ = torch.tanh(self.Wc(hx))

            # 遗忘门
            ft = torch.sigmoid(self.Wf(hx))

            # 记忆更新
            cell_state = ft.mul(cell_state) + it.mul( ct_)

            # 输出门
            ot = torch.sigmoid(self.Wo(hx))
            # ht=ot@torch.tanh(self.cell_state)
            hidden_state = ot.mul( torch.tanh(
                cell_state) ) # outputs : [batch_size, num_directions(=1) * n_hidden]

            ##第二层
            hx2 = torch.cat((hidden_state, hidden_state2),1)
            it2 = torch.sigmoid(self.Wi2(hx2))
            ct_2 = torch.tanh(self.Wc2(hx2))

            # 遗忘门
            ft2 = torch.sigmoid(self.Wf2(hx2))

            # 记忆更新
            cell_state2 = ft2.mul(cell_state2) + it2.mul( ct_2)

            # 输出门
            ot2 = torch.sigmoid(self.Wo2(hx2))
            hidden_state2= ot2.mul( torch.tanh(
                cell_state2))  # outputs : [n_step, batch_size, num_directions(=1) * n_hidden]
